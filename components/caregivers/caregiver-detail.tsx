...
export function CaregiverDetail({
  ...
  schedules = [
    { id: "1", patientName: "이환자", days: ["월", "수", "금"], time: "09:00 - 12:00" },
    { id: "2", patientName: "최환자", days: ["화", "목"], time: "14:00 - 17:00" },
  ],
  ...
  return (
    <PageContainer>
      <div className="flex items-center mb-6">
        ...
        <PageHeader title={`${name} 요양보호사`} description="요양보호사 상세 정보" className="mb-0 flex-1" />
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="card-shadow md:col-span-1">
          ...
          <CardContent className="flex flex-col items-center text-center">
            ...
            <p className="text-lg text-muted-foreground mb-4">요양보호사 / {age}세</p>
        ...
        <CardContent>
          <div className="space-y-4">
            {schedules.length > 0 ? (
              schedules.map((schedule) => (
                <div key={schedule.id} className="p-4 border rounded-lg">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="text-lg font-medium">{schedule.patientName} 님</h3>
                  ...
                  <div className="flex gap-2">
                    {schedule.days.map((day) => (
                      <span key={day} className="px-3 py-1 bg-primary/10 text-primary rounded-full text-sm">
                        {day}
                      </span>
                    ))}
                  </div>
              ...
              <p className="text-center text-muted-foreground py-8">등록된 근무 일정이 없습니다</p>
            )}
          </div>
        </CardContent>
      </Card>
