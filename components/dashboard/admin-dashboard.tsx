...
export function AdminDashboard({
  adminName = "관리자",
  schedules = [
    { id: "1", caregiverName: "김요양", patientName: "이환자", time: "09:00 - 12:00", status: "completed" },
    { id: "2", caregiverName: "박요양", patientName: "최환자", time: "12:00 - 15:00", status: "in-progress" },
    { id: "3", caregiverName: "정요양", patientName: "강환자", time: "15:00 - 18:00", status: "scheduled" },
  ],
  notifications = [
    { id: "1", message: "김요양 보호사가 출근 소명을 요청했습니다", time: "10분 전" },
    { id: "2", message: "박요양 보호사가 돌봄 일지를 작성했습니다", time: "30분 전" },
  ],
}: AdminDashboardProps) {
  ...
  return (
    <PageContainer>
      <PageHeader title={`${adminName}님, 안녕하세요`} description="오늘의 스케줄과 알림을 확인하세요" />
          ...
          <CardContent>
            <div className="space-y-4">
              {notifications.length > 0 ? (
                ...
                <p className="text-center text-muted-foreground py-4">새로운 알림이 없습니다</p>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <div className="space-y-4">
            {schedules.map((schedule) => (
              <div key={schedule.id} className="p-4 border rounded-lg">
                ...
                <p className="text-base text-muted-foreground">{schedule.time}</p>
              </div>
            ))}
          </div>
        </CardContent>
